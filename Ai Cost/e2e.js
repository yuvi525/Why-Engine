...
async function runTests() {
  ...
  try {
    const complexText = 'Analyze this comprehensive data and calculate step-by-step the architecture optimizations required for a microservices refactor with multiple import statements and class definitions. '.repeat(10);
  ...
  await prisma.budgetState.update({
    ...
    data: { spentTodayMicro: 4500000, dailyLimitMicro: 5000000, autoDowngradeAt: 0.8 } // 90% spent
  });

  if (redisUrl && redisToken) {
    await fetch(`${redisUrl}/hset/budget:${USER_ID}:today/spentMicro/4500000`, {
  ...
  if (redisUrl && redisToken) {
    await fetch(`${redisUrl}/hset/budget:${USER_ID}:today/spentMicro/0`, {
      headers: { Authorization: `Bearer ${redisToken}` }
    });
  }
  ...
  // The seed script created 10, and we just did 3 requests (plus the one from test.js)
